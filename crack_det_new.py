...
model_file = ("/Users/Roman_Mysiuk/Downloads/Crack-Semantic-Segmentation-master_2/model_files/onnx_model_name.onnx")
...
while True:
	...
	# Завантаження моделі з рядка байтів
	model = rt.InferenceSession(model_bytes)

	# Збереження моделі у тимчасовий файл
	with open('temp_model.onnx', 'wb') as f:
  	  f.write(model_bytes)

	# Запуск Netron для відображення моделі
	netron.start('temp_model.onnx')
	...
	output = ((0.4 * frame) + (0.5 * mask)).astype("uint8")
	...
	if writer is None:
		...
		#writer = cv2.VideoWriter(args["output"], fourcc, 10,(1280, 720), True)
